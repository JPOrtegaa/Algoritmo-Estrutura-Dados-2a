...
void inserir(TabelaHash tabela, int k, Registro reg){
    ...
    for(i = 0; i < TAM; i++) // condicao de parada, i < m???
        if(!tabela[(hash + i*i) % TAM].ocupado){ // se nao estiver ocupado o slot, insira!
            tabela[(hash + i*i) % TAM].k = k; // ou k + i, alterar a chave ou nao???
            tabela[(hash + i*i) % TAM].reg = reg;
            tabela[(hash + i*i) % TAM].ocupado = 1;
            return;
        }
...
void remover(TabelaHash tabela, int k){
    ...
    for(i = 0; i < TAM; i++)
        if(tabela[(hash + i*i) % TAM].k == k){
            tabela[(hash + i*i) % TAM].k = 0;
            tabela[(hash + i*i) % TAM].reg.matricula = 0;
            strcpy(tabela[(hash + i*i) % TAM].reg.nome, "\0");
            tabela[(hash + i*i) % TAM].ocupado = 0;
            return;
        }
}
